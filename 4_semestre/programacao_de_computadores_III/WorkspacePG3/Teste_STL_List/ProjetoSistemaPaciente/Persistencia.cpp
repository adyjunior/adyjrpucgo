...
Persistencia::Persistencia(std::string const &nomeDoArquivo){
	...
	//if(!arquivoDeEntrada.is_open()) arquivoDeEntrada.open(nomeDoArquivo.c_str());
...
void Persistencia::apagar(std::string const &nome)const{
	Paciente p;
	...
	try{
		...
		for(int i=0;i < tam;i++){
			...
			if(p.getNome()== nome) posicao = i;   //std::cout<<"POSICAO NO IF -> ( "<<i<<" ) = "<<vet[i]<<std::endl;}
		}
...
bool Persistencia::buscar(std::string const &nome)const{
