...
int CDC::converteStrParaInteiro(std::string const &str) {
	...
	for(unsigned int pos=0;pos<str.length();pos++){
		if(static_cast<int>(str[pos]) < 48 || static_cast<int>(str[pos]) > 57) throw (0);// Erro 0 , Caracter invalido Digitados no campo de numero
...
float CDC::converteStrParaReal(std::string const &str){
	...
	bool teste = true;
	for(unsigned int pos=0;pos<str.length();pos++){
		if((static_cast<int>(str[pos])< 48 || static_cast<int>(str[pos]) > 57) && str[pos]!='.') throw (0);// Erro 0 , Caracter invalido Digitados no campo de numero
...
std::string CDC::buscarSubString(std::string const &str, unsigned int &pos, char caracter){
	if(pos>=str.length()) throw (3);// Erro 3 Posicao digitada esta fora do intervalo da string
	std::string strSaida="";
